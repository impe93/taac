#!/usr/bin/env bash
#
# prepare-asr-runtime.sh — build the bundled Python runtime for the realtime
# ASR sidecar (Qwen3-ASR via MLX, macOS Apple Silicon only).
#
# Downloads python-build-standalone (arm64), installs the pinned ASR library
# into it and prunes what the sidecar does not need. The resulting
...
if [[ -x "${RUNTIME_DIR}/bin/python3" ]] \
  && "${RUNTIME_DIR}/bin/python3" -c "import mlx_qwen3_asr" >/dev/null 2>&1; then
...
echo "[prepare-asr-runtime] Installing ${ASR_PACKAGE}..."
"${RUNTIME_DIR}/bin/python3" -m pip install --no-cache-dir --disable-pip-version-check "${ASR_PACKAGE}"
...
"${RUNTIME_DIR}/bin/python3" -c "import mlx_qwen3_asr, numpy; print('mlx-qwen3-asr OK')"
